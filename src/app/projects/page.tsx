        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)}
                className="w-full px-3 py-2 rounded-lg border text-sm" placeholder="z.B. Cloud-Migration" required />
            </div>
            <div>
                  ...
                  <button key={t} type="button" onClick={() => setFormType(t)}
                    className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold border transition-all ${
                      formType === t
                        ...
                        : 'border-black/[0.06] dark:border-white/[0.06] dark:text-white/50 text-gray-500'
                    }`}
                    style={formType === t ? { color: PROJECT_TYPE_CONFIG[t].color, background: `${PROJECT_TYPE_CONFIG[t].color}15` } : {}}>
            ...
            {formType === 'external' && (
              <div>
                ...
                <input type="text" value={formClient} onChange={(e) => setFormClient(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg border text-sm" placeholder="z.B. BMW AG" />
              </div>
            )}
            <div>
          ...
          <div>
            ...
            <input type="text" value={formDesc} onChange={(e) => setFormDesc(e.target.value)}
              className="w-full px-3 py-2 rounded-lg border text-sm" placeholder="Optional" />
          </div>
                ...
                <button key={m.id} type="button" onClick={() => toggleMember(m.id)}
                  className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-all ${
                    formMembers.includes(m.id)
                      ...
                      : 'border-black/[0.06] dark:border-white/[0.06] dark:text-white/50 text-gray-600 hover:border-indigo-500/20'
                  }`}>
                  {m.name}
                </button>
              ))}
          ...
          <div className="flex gap-3">
            <button type="submit"
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-500 text-white text-sm font-semibold hover:bg-indigo-600 transition-colors">
              <Check size={14} />
      ...
      {filtered.length === 0 && !showForm ? (
        ...
        <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 gap-4' : 'space-y-4'}>
          {filtered.map((project) => {
            ...
            return (
              <div key={project.id}
                className="card-shimmer rounded-xl border border-black/[0.06] dark:border-white/[0.06] p-5 hover:border-indigo-500/20 transition-all">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {project.description && (
                      <p className="text-xs dark:text-white/30 text-gray-400 mt-1 line-clamp-2">{project.description}</p>
                    )}
                  </div>
                ...
                {(project.startDate || project.endDate) && (
                  <div className="flex items-center gap-1.5 mt-3">
                    ...
                    <span className="text-[11px] dark:text-white/30 text-gray-400">
                      {project.startDate ? new Date(project.startDate).toLocaleDateString('de-DE', { day: '2-digit', month: 'short', year: 'numeric' }) : '–'}
                      {' → '}
                      {project.endDate ? new Date(project.endDate).toLocaleDateString('de-DE', { day: '2-digit', month: 'short', year: 'numeric' }) : '–'}
                    </span>
                  </div>
                )}
