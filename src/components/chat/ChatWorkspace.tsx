      ...
      <aside className="w-64 shrink-0 flex flex-col border-r dark:border-white/6 border-gray-200 bg-gray-50 dark:bg-gray-900/50">
        ...
        <div className="px-4 py-3 border-b dark:border-white/6 border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {totalUnread > 0 && (
              <span className="text-[9px] font-black px-1.5 py-0.5 rounded-full bg-(--primary) text-white">{totalUnread}</span>
            )}
          </div>
          <button className="p-1 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors border-none bg-transparent cursor-pointer dark:text-white/40 text-gray-500">
            <Plus size={14} />
          </button>
        </div>
        ...
        <div className="px-3 py-2">
          <div className="relative">
            ...
            <input value={sidebarSearch} onChange={e => setSidebarSearch(e.target.value)} placeholder="Suchen..."
              className="w-full pl-8 pr-3 py-1.5 rounded-lg text-xs bg-black/4 dark:bg-white/4 border-none outline-none dark:text-white text-gray-900 dark:placeholder-white/30 placeholder-gray-400" />
          </div>
        </div>
          ...
          <div>
            ...
            {channelList.map((ch) => (
              <button key={ch.id} onClick={() => setActiveChannelId(ch.id)}
                ...
                {(ch.unread ?? 0) > 0 && (
                  <span className="text-[8px] font-black px-1.5 py-0.5 rounded-full bg-(--primary) text-white shrink-0">{ch.unread}</span>
                )}
                ...
                <button key={dm.id} onClick={() => setActiveChannelId(dm.id)}
                  ...
                  <div className="relative w-5 h-5 rounded-full bg-(--primary-light) flex items-center justify-center shrink-0 overflow-hidden">
                    {member?.avatarUrl ? (
                      ...
                      <span className="text-[8px] font-black text-(--primary)">{dm.name.charAt(0)}</span>
                    )}
                  ...
                  {(dm.unread ?? 0) > 0 && (
                    <span className="text-[8px] font-black px-1.5 py-0.5 rounded-full bg-(--primary) text-white shrink-0">{dm.unread}</span>
                  )}
                </button>
              );
            })}
        ...
        {/* Me */}
        <div className="p-3 border-t dark:border-white/6 border-gray-200 flex items-center gap-2">
          <div className="w-7 h-7 rounded-full bg-(--primary-light) flex items-center justify-center text-(--primary) font-black text-xs shrink-0">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="flex items-center justify-between px-5 py-3 border-b dark:border-white/6 border-gray-200 bg-white dark:bg-gray-950 shrink-0">
          <div className="flex items-center gap-3">
            {activeChannel?.type === 'channel' ? (
              ...
              <div className="w-8 h-8 rounded-full bg-(--primary-light) flex items-center justify-center text-(--primary) font-black text-sm">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setShowMembers(v => !v)}
              className={`p-2 rounded-lg border-none cursor-pointer transition-colors ${showMembers ? 'bg-(--primary-light) text-(--primary)' : 'bg-transparent dark:text-white/40 text-gray-500 hover:bg-black/4 dark:hover:bg-white/4'}`} title="Mitglieder">
              <Users size={16} />
            </button>
            <button className="p-2 rounded-lg border-none cursor-pointer transition-colors bg-transparent dark:text-white/40 text-gray-500 hover:bg-black/4 dark:hover:bg-white/4" title="Suchen">
              <Search size={16} />
            </button>
            <button className="p-2 rounded-lg border-none cursor-pointer transition-colors bg-transparent dark:text-white/40 text-gray-500 hover:bg-black/4 dark:hover:bg-white/4" title="Weitere Optionen">
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 flex flex-col min-w-0">
                    ...
                    {group.messages.map((msg, idx) => {
                      ...
                      if (isSystem) {
                        return (
                          <div key={msg.id} className="flex justify-center my-2">
                            <span className="text-[10px] px-3 py-1 rounded-full dark:bg-white/4 bg-gray-100 dark:text-white/40 text-gray-500">{msg.text}</span>
                          </div>
                        );
                      }
                      ...
                      return (
                        <div key={msg.id} className={`flex gap-3 ${mine ? 'flex-row-reverse' : ''} ${showSender ? 'mt-3' : 'mt-0.5'} group`}>
                          {/* Avatar */}
                          {showSender && !mine && (
                            <div className="w-8 h-8 rounded-full bg-(--primary-light) flex items-center justify-center text-(--primary) font-black text-xs shrink-0 mt-1">
                          ...
                          <div className={`max-w-[70%] ${mine ? 'items-end' : 'items-start'} flex flex-col`}>
                            ...
                            <div className={`relative px-3 py-2 rounded-2xl text-sm leading-relaxed ${mine
                              ...
                              : 'dark:bg-white/6 bg-gray-100 dark:text-white text-gray-900 rounded-tl-sm'}`}>
                              {msg.text}
                              {mine && (
                                <div className="absolute -bottom-3.5 right-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                  {msg.read ? <CheckCheck size={10} className="text-(--primary)" /> : <Check size={10} className="text-gray-400" />}
                                </div>
                              )}
                            </div>
                          </div>
            ...
            {/* ── Input ──────────────────────────────── */}
            <div className="px-5 py-3 border-t dark:border-white/6 border-gray-200 bg-white dark:bg-gray-950 shrink-0">
              <div className="flex items-end gap-3 p-3 rounded-2xl border dark:border-white/8 border-gray-200 dark:bg-white/2 bg-gray-50 transition-colors focus-within:border-(--primary)">
                <button className="p-1 rounded-lg border-none bg-transparent cursor-pointer dark:text-white/30 text-gray-400 hover:text-(--primary) transition-colors shrink-0 mb-0.5">
                  <Plus size={18} />
                </button>
                <textarea
                  ref={inputRef}
                ...
                <div className="flex items-center gap-1 shrink-0 mb-0.5">
                  <button className="p-1.5 rounded-lg border-none bg-transparent cursor-pointer dark:text-white/30 text-gray-400 hover:text-(--primary) transition-colors">
                    <Smile size={16} />
                  </button>
                  <button className="p-1.5 rounded-lg border-none bg-transparent cursor-pointer dark:text-white/30 text-gray-400 hover:text-(--primary) transition-colors">
                    <Paperclip size={16} />
                  ...
                  <button onClick={sendMessage} disabled={!input.trim() || sending}
                    className="p-1.5 rounded-lg border-none cursor-pointer transition-all disabled:opacity-30 bg-(--primary) text-white hover:opacity-90 disabled:cursor-not-allowed">
                    {sending ? <Loader size={16} className="animate-spin" /> : <Send size={16} />}
                  </button>
                </div>
              </div>
              <div className="mt-1.5 text-[9px] dark:text-white/20 text-gray-400 text-center">
                <kbd className="px-1 py-0.5 rounded bg-black/6 dark:bg-white/6 font-mono">Enter</kbd> senden &nbsp;·&nbsp;
                <kbd className="px-1 py-0.5 rounded bg-black/6 dark:bg-white/6 font-mono">Shift+Enter</kbd> neue Zeile
              </div>
            </div>
          </div>
          ...
          {/* ── Members Panel ─────────────────────────── */}
          {showMembers && (
            <aside className="w-56 shrink-0 border-l dark:border-white/6 border-gray-200 bg-gray-50 dark:bg-gray-900/50 flex flex-col overflow-hidden">
              <div className="px-4 py-3 border-b dark:border-white/6 border-gray-200 flex items-center justify-between">
                <span className="text-xs font-black dark:text-white text-gray-900">Mitglieder</span>
                <button onClick={() => setShowMembers(false)} className="p-1 rounded-lg border-none bg-transparent cursor-pointer dark:text-white/40 text-gray-500 hover:bg-black/4 dark:hover:bg-white/4">
                  <X size={13} />
                </button>
              </div>
              <div className="flex-1 overflow-y-auto custom-scrollbar px-2 py-2 space-y-0.5">
                ...
                {members.slice(0, 3).map((m) => (
                  <div key={m.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-black/4 dark:hover:bg-white/4 cursor-pointer">
                    <div className="relative w-6 h-6 rounded-full bg-(--primary-light) flex items-center justify-center text-(--primary) font-black text-[9px] shrink-0 overflow-hidden">
                ...
                {members.length > 3 && (
                  <>
                    ...
                    {members.slice(3).map((m) => (
                      <div key={m.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-black/4 dark:hover:bg-white/4 cursor-pointer opacity-50">
